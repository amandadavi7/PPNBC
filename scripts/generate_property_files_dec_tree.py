...
def process_return(return_statement):
    split_list = return_statement.split(' ')
    counts = []
    denominator = 0
    for i in range(classlabels):
        num = int((split_list[i+2]).split('.')[0])
        counts.append(num)
        denominator = denominator + num

    for i in range(classlabels):
        counts[i] = float(counts[i])/denominator	
    return counts 
...
if __name__ == "__main__":
    ...
    od = collections.OrderedDict(sorted(tree_node_index_value_mapping.items()))
    leaf_to_class_index = [None] * pow(2, depth)
    
	# map leaf nodes to classes
