...
. "scripts/lr_client.sh" $totaltests "damf-test"
...
do
   ...
   do  
	...
	do
            ...
            java -cp build/classes/ Party.Party party_port=$(($startPort+100)) ti=127.0.0.1:3000 ba=127.0.0.1:4000 party_id=$j yCsv="$yCsvFile" xCsv="$xCsvFile" oneShares=0 output=$outputpath model=2 partyCount=$i &
	    pids[$((2+$j))]=$!
